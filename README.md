...
3. Each flow's placement function (`resolveVertical` / `resolveHorizontal` /
   ...
   to spare.
4. The output (`ResolvedLayout`) is a flat array of `{ id, x, y, width,
   height, visible }` — a renderer never has to know *why* something is
...
- `AdElement.role` is a closed union (`"primary" | "hero" | "action" |
  "secondary" | "branding"`) — an unknown role is a compile-time error, not a
...
~1 day, spread across algorithm design, implementation, and testing.
